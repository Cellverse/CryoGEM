...
from cryogem.inference.utils_infer import write_image, read_image
...
def add_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--feature_vector_object", type=str, default="phy", choices=["phy", "syn", "real"], 
        help="select feature vector object, physical_based(input of cryogem), synthetic(output of cryogem), real(real micrograph,without mask provided)")
    #! feature map selector 
    parser.add_argument("--feature_map_object", type=str, default="syn", choices=["phy", "syn", "real"], 
        help="select feature map object, physical_based(input of cryogem), synthetic(output of cryogem), real(real micrograph,without mask provided)")
...
WINDOW_NAME = "Analysis of CryoGEM's Latent Space"
...
def main(args):
    ...
    if feature_vector_object != 'real' or feature_map_object != 'real':
        ...
        physical_based = cryogem.apply_physical_based_function(
            clean_mrc,
            inverse_colour=True,
            to_numpy=True,
        )
    ...
    vector_features = cryogem.get_feature(feature_vector_image)
    map_features = cryogem.get_feature(feature_map_image)
    
    feat_vec_pool_pos, feat_vec_pool_neg, _, _ = cryogem.adaptor(vector_features, num_patches=1, pos_grids=pos_grids, neg_grids=neg_grids, l2_norm=True, use_mlp=True)
    feat_map_pool = cryogem.adaptor.custom_forward(map_features, l2_norm=True, use_mlp=True)
