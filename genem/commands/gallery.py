...
def main(args):
    ...
    if args.deform:
        ...
        rotations = genem.get_vol().gen_same_rotations(n_particles)
        z_vals = genem.get_vol().gen_z_vals(n_particles, linear=True)
        particles = genem.get_vol()(rotations, z_vals=z_vals)
    ...
    if args.defocus:
        ...
        rotations = genem.get_vol().gen_rotations(1)
        particles = genem.get_vol()(rotations)
        ctf_params = generate_sequential_ctf_params(args.n_frames, df_min=args.df_min, df_max=args.df_max)
        freqs_mag, angles_rad = compute_safe_freqs(size, genem.apix)
